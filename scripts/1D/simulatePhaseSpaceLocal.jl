...
tmax = 500
xmaxVect::Vector{Int64} = max.(500, round.(1.5 .* vFKKPVect .* tmax, digits = -2))
...
for r in rVect, (idxR0, R0) in enumerate(R0Vect)
    ...
    (nx0, hx0, x) = getInitialCondition("steadyState", R0, r, mutationRate, mutationKernel, Nh, xmaxVect[idxR0])
    (nx, hx) = simulateWave(nx0, hx0, R0, r, Nh, mutationRate, mutationKernel, dt, tmax, dtSampling, x)
    saveSimulation(nx, hx, r, R0, mutationRate, mutationKernel, tmax, dt, xmaxVect[idxR0])
    plotSimulationSummary(nx, hx, xmaxVect[idxR0])
end