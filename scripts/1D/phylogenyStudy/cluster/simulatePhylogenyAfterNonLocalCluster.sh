...
MUSIZE=1
NONLOCPROBSIZE=1
TAFTERNONLOCALSIZE=10
RUNSIZE=200
TOTAL_LINES=$(( MUSIZE * NONLOCPROBSIZE * RUNSIZE * TAFTERNONLOCALSIZE))
NJOBS=100
...
while IFS= read -r line; do
    if (( i >= START && i < END )); then
        ...
        read r R0 mu localKernel Delta nonLocalProb Nh tmax xmax tAfterNonLocal run <<< "$line"

        echo "Job $SLURM_ARRAY_TASK_ID running parameters: r=$r, R0=$R0, mu=$mu, localKernel=$localKernel, Delta=$Delta, nonLocalProb=$nonLocalProb, Nh=$Nh, tmax=$tmax, xmax=$xmax, tAfterNonLocal =$tAfterNonLocal, run=$run"
