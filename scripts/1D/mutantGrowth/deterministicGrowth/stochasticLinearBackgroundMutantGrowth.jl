...
# xp = plot(tSampling, xm, xlabel = raw"$t$", c = :steelblue, ylabel = raw"$\langle x_m\rangle(t)$", label = "Simulation results")
# plot!(xp, tSampling, x0 .+ (s*varm[1]).*tSampling .+ (s*D).*tSampling.^2, colour = :black, label = "Gaussian approximate solution")
# sigmap = plot(tSampling, varm,  xlabel = raw"$t$", c = :steelblue, ylabel = raw"$\sigma_m^2(t)$", label = "Simulation results")
# plot!(sigmap,tSampling, varm[1] .+ 2D.*tSampling, c = :black, label = "Gaussian approximate solution")
# plot!(sigmap, [],[], c = :coral, label = raw"Non-Gaussian correction $sm_3(t)/2D$")
# plot!(twinx(), tSampling, abs.(s.*m3 ./ 2D), c = :coral, ylabel = "Non-Gaussian correction ratio", label = "", ylims = (0,1))

# display(Mp)
# display(xp)
# display(sigmap)

# # Plots for multiple M0 display
# xp = plot(tSampling, xmGrid[1:idxMax,:]', xlabel = raw"$t$", palette = palette(:viridis, idxMax), ylabel = raw"$\langle x_m\rangle(t)$", label = permutedims(["M0 = $(M0Grid[i])" for i in eachindex(M0Grid)][1:idxMax]))
# plot!(xp, tSampling, x0 .+ (s*v0).*tSampling .+ (s*D).*tSampling.^2, colour = :coral, label = "Gaussian approximate solution", lw = 2)
# sigmap = plot(tSampling, varmGrid[1:idxMax,:]',  xlabel = raw"$t$", palette = palette(:viridis, idxMax), ylabel = raw"$\sigma_m^2(t)$", label = permutedims(["M0 = $(M0Grid[i])" for i in eachindex(M0Grid)][1:idxMax]))
# plot!(sigmap,tSampling, v0 .+ 2D.*tSampling, c = :coral, label = "Gaussian approximate solution", lw = 2)
...
plot!(tSampling, avM, c = :black, lw = 2, label = "Simulation average")
plot!(tSampling, Mintegrated, c = :steelblue, lw = 2, label = "Integrated equation")
plot!(tSampling, Mtheo, c = :coral, lw = 2, label = "Gaussian theory")
...
plot!(avmSurvivedp, x, avmtheo ./ (sum(isnan.(tExtinction)) / nRuns), label = raw"Theoretical result over $\mathbb{P}_\mathrm{s}$", c = :coral, lw = 2)

# Coverage plot
plot(tSampling, replace(cmGrid,0=>NaN)',  xlabel = raw"$t$", c = :gray, ylabel = raw"$c(\bar{x}_m(t),t)$", label = permutedims(["" for i in 1:nRuns]), alpha = 0.4, yscale = :log10)
...
plot!(tSampling, avM ./ (s.*(x0 .- v.*tSampling)), c = :steelblue, lw = 2, label = "Theoretical coverage")
